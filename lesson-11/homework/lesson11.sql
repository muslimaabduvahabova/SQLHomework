...
SELECT Departments.DepartmentName, Employees.Name as TopEmployeeName, MAX(Employees.Salary) as MaxSalary
FROM Employees
JOIN Departments ON Employees.DepartmentID = Departments.DepartmentID
...
from Customers
left join Orders on Customers.CustomerID = Orders.CustomerID
group by Customers.Firstname, Orders.OrderID
...
from Orders
join Customers on Orders.CustomerID = Customers.CustomerID
where Customers.Country = 'USA' and Customers.Address like '____%'
...
from Sales
join Products on Sales.ProductID = Products.ProductID
where Products.Category = Sales.SaleAmount > 350
